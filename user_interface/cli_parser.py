...
def validate_format_inputs(args: argparse.Namespace, input_output_flags: 'InputOutputFlags') -> Tuple[Optional[str], Optional[str]]:
    """
    ...
    """
    input_ext = validate_input_format(args)
    output_ext = validate_output_format(args)
...
def validate_input_format(args: argparse.Namespace) -> Optional[str]:
    """
    Validate input format.
    """
    if args.input_format:
        if args.input_format in ALIAS_TO_EXTENSION_MAP:
            input_ext = ALIAS_TO_EXTENSION_MAP[args.input_format]
        else:
            logging.error(
                f"Received invalid input format: {args.input_format}\n Input format will be automatically detected."
            )
            input_ext = None
    else:
        input_ext = None
...
def validate_output_format(args: argparse.Namespace) -> Optional[str]:
    """
    Validate output format.
    """
    if args.output_format:
        if args.output_format in ALIAS_TO_EXTENSION_MAP:
            output_ext = ALIAS_TO_EXTENSION_MAP[args.output_format]
        else:
            logging.error(
                f"Received invalid output format: {args.output_format}"
            )
            output_ext = None
    else:
        output_ext = None
